...
# Time: O(F) + O(C) = O(n) 
#       F is # of nodes to start of cycle
#       C is # of nodes in cycle
#       n is total number of nodes
# Space: O(1)
def has_cycle(head: ListNode) -> Optional[ListNode]:
    def cycle_len(end):
        start, length = end.next, 1
...
# Hash table -> need to make ListNode hashable
# TODO
# Time: O(n) Space: O(n)
def has_cycleh(head: ListNode) -> Optional[ListNode]:
    visited = set() 
    while head:
        if head in visited:
            return head
